...
def fetch_optimizer(args, model):
    ...
    params = get_default_optimizer_params(model, weight_decay=0., weight_decay_norm=0.)
    optimizer = torch.optim.Adam(params, args.lr)
    scheduler = WarmupPolyLR(optimizer, args.num_steps,
                             warmup_factor=0.001, warmup_iters=1000, warmup_method='linear',
                             power=0.9, constant_ending=0.0)
